...
%%%-------------------------------------------------------------------
%%% @doc
%%%  This is a generic FSM in the spirit of OTP's gen_fsm but subtly
%%%  different. A generic server process (jhn_fsm) implemented using this
%%%  module has a standard set of interface functions.
...
%%%
%%%   If a callback function fails or returns a bad value, the jhn_fsm
%%%   process terminates. N.B. that a gen_fsm process does not trap exit
...
%%%
%%%   The event callback function is called when the state function for the
%%%   does not have a matching clause for the incoming event, sent either by
...
%%%
%%%   N.B. Recieving an event/message that is deferred will break hibernation,
...
%% @doc
%%   A synchronization is made with FSM with Timeout. Will generate a timeout
%%   exception if the FSM takes more than Timeout time to answer.
%%   Generates an exception if the process is dead, dies, or no process
...
%% @doc
%%   Called inside a state function or event/2 and it tells if it is a call
%%   or a cast.
%% @end
%%--------------------------------------------------------------------
-spec type() -> call | cast.
...
%% @private
%%--------------------------------------------------------------------
-spec request(event(), #state{}) -> jhn_server:return(#state{}).
%%--------------------------------------------------------------------
request(Event, State) ->
...
%% @private
%%--------------------------------------------------------------------
-spec message(event(), #state{}) -> jhn_server:return(#state{}).
%%--------------------------------------------------------------------
...
%% @private
%%--------------------------------------------------------------------
-spec code_change(_, #state{}, _) -> no_return().
%%--------------------------------------------------------------------
