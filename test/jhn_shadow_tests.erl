...
load_app_2_test_() ->
    BadSpec1 = #{env => [{1, one}],
                  ...
                  sup => {1, 2, 3},
                  foo => bar},
    BadSpec2 = #{sup => {app, 2, 3}, mod => fnu},
    BadSpec3 = #{sup => {app, mod, 3}},
    BadSpec4 = #{sup => {1, 2, 3, 4}},
    BadSpec5 = #{sup => {app, 2, 3, 4}},
    BadSpec6 = #{sup => {app, mod, 3, 4}},
    BadSpec7 = #{sup => {app, mod, func, 4}},
    BadSpec8 = #{sup => fnu},
    {setup,
     fun() -> ok end,
     fun(_) -> ok end,
    [?_test(?assertMatch(ok, jhn_shadow:load_app(foo, [{one, 1}]))),
     ...
     ?_test(?assertMatch({error, _}, jhn_shadow:load_app(foo, BadSpec1))),
     ?_test(?assertMatch({error, _}, jhn_shadow:load_app(foo, BadSpec2))),
     ?_test(?assertMatch({error, _}, jhn_shadow:load_app(foo, BadSpec3))),
     ?_test(?assertMatch({error, _}, jhn_shadow:load_app(foo, BadSpec4))),
     ?_test(?assertMatch({error, _}, jhn_shadow:load_app(foo, BadSpec5))),
     ?_test(?assertMatch({error, _}, jhn_shadow:load_app(foo, BadSpec6))),
     ?_test(?assertMatch({error, _}, jhn_shadow:load_app(foo, BadSpec7))),
     ?_test(?assertMatch({error, _}, jhn_shadow:load_app(foo, BadSpec8)))
    ]}.
